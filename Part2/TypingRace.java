...
public class TypingRace
{
   ...
   private int numTypists;// keep track of selected typists
    private boolean[] justMistyped; //created my own variable
    ...
    public TypingRace(int passageLength, int numTypists)
    {
        ...
        justMistyped=new boolean[numTypists];
    }
    ...
    {
        if (seatNumber >=1 && seatNumber <= numTypists)
        {
    ...
    public void startRace()
    {
        ...
        while (!finished)
        {
            ...
            //CAN HAVE NULL POINTER EXXEPTIONS AS SEAT COULD BE EMPTY
            for (int i = 0; i < numTypists; i++)
            {
                if(typists[i]!=null)
                {
            ...
            for (int i = 0; i < numTypists; i++)
            {
                if(typists[i]!=null && raceFinishedBy(typists[i]))
                {
                    winner=typists[i];
                    finished = true;
                }
                ...
                System.out.println("the accuracy is " + winner.getAccuracy());
            }
     ...
     */
    private void advanceTypist(Typist theTypist, int index)
    {
        ...
        if (Math.random() < (1-theTypist.getAccuracy()) * MISTYPE_BASE_CHANCE) // should be 1-accuracy as the chance of a mistype from a hugh accuracy should be low
        { //need to print out just mistyped
            theTypist.slideBack(SLIDE_BACK_AMOUNT);
            justMistyped[index]=true;
            System.out.println(theTypist.getName() + " has JUST MISTYPED");
        }
     ...
     */
    private void printSeat(Typist theTypist,int index)
    {
        ...
        if(justMistyped[index])
        {
            System.out.println('<');
            justMistyped[index]=false;
        }
